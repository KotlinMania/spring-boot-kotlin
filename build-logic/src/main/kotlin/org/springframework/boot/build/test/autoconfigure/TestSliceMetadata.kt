 ...
 */
@JvmRecord
internal data class TestSliceMetadata(val module: String?, val testSlices: MutableList<TestSlice?>?) {
    fun writeTo(file: File?) {
        JsonMapper.builder().enable(SerializationFeature.INDENT_OUTPUT).build().writeValue(file, this)
    }

    @JvmRecord
    internal data class TestSlice(val annotation: String?, val importedAutoConfigurations: MutableList<String?>?)
