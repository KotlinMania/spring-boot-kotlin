 ...
 */
internal class ConfigurationPropertiesAnalyzer(sources: MutableCollection<File>) {
    ...
    internal class Report(private val baseDirectory: File) {
    ...
    internal class Analysis(private val header: String?) {
    ...
    companion object {
        ...
        private fun <T> writeAll(writer: PrintWriter, elements: Iterable<T?>, itemWriter: Consumer<T?>) {
            val it: MutableIterator<T?> = elements.iterator()
            while (it.hasNext()) {
