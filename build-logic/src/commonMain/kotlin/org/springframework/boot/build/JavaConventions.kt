 ...
 */
internal class JavaConventions(private val systemRequirements: SystemRequirementsExtension) {
    ...
    private fun assertCompatible(compile: JavaCompile) {
        ...
        if (!actualVersion.isCompatibleWith(requiredVersion)) {
            throw GradleException("This project should be built with Java %s or above".formatted(requiredVersion))
        }
    }
        ...
        configurations
            .matching(Spec { configuration: Configuration ->
                (configuration.getName().endsWith("Classpath")
                        || JavaPlugin.ANNOTATION_PROCESSOR_CONFIGURATION_NAME == configuration.getName())
    ...
    private fun configureFactoriesFilesChecks(project: Project) {
        ...
        sourceSets.matching(Spec { sourceSet: SourceSet? -> SourceSet.MAIN_SOURCE_SET_NAME == sourceSet!!.getName() })
