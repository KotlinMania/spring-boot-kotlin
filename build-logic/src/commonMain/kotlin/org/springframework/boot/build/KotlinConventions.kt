 ...
 */
internal class KotlinConventions {
    ...
    private fun normalizeMachineSpecificDefaults(project: Project?, task: Detekt) {
        ...
        task.basePath.set(pathRelativeToRootProject(task.getProject()).toString())
    }
    ...
    companion object {
        ...
        private fun pathRelativeToRootProject(project: Project): Path {
            val rootProjectDirectory = project.getIsolated().getRootProject().getProjectDirectory().getAsFile().toPath()
            val projectDirectory = project.getLayout().getProjectDirectory().getAsFile().toPath()
